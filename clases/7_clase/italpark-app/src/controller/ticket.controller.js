...
export const TicketController = {
	...
	ticketsListingAll: async (req, res) => {
		const ticketsListingAll = await TicketService.serviceTicketAll();
		console.log(ticketsListingAll);

		if (!ticketsListingAll) {
			res.status(404).json({
				payload: null,
				message: "No se pudo actualizar nada",
				ok: false,
		...
		res.status(200).json({
			message: `Tenes todos los tickets`,
			payload: ticketsListingAll,
			ok: true,
		});
		return;
	},
